...
// ** my original code for finding identical subtree
...
//   if (!subtree && !tree) return true;
//   if (!subtree || !tree) return false;
...
// ** optimized code for finding identical subtree
function isIdentical(subtree, tree) {
  if (!subtree && !tree) return true;
  if (!subtree || !tree) return false;
  return (
...
function isSubtree(subtree, tree = this.root) {
  ...
  if (subtree.value === tree.value && isIdentical(subtree, tree)) return true;
